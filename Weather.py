...
def coords_to_gridpoints(coordinates):
    ...
    location_data = coordinate_data["relativeLocation"]["properties"]
    location = f"{location_data["city"]}, {location_data["state"]}"
    station__coords_and_location = (coordinate_data["gridId"], coordinate_data["gridX"], coordinate_data["gridY"], location)
    return station__coords_and_location
...
def forecast_from_gridpoints(station_and_coords):
    ...
    return (weather, temp, station_and_coords[3])
